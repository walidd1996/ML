...
y_cv=y_cv/max(y_cv);
 ...
 %%%% linear%%
for i=1:5000
    sum_cost =(train_linear'*(train_linear*theata_linear-y_train));
    theata_linear=theata_linear-alpha*(sum_cost/m);   
    h = train_linear*theata_linear;
    j_linear_train(i)=mean((h-y_train).^2);    
    
end


for i=1:5000  
    sum_cost =alpha*(cv_linear'*(cv_linear*theata_linear-y_cv));
    theata_linear=theata_linear-sum_cost/n;   
    h = cv_linear*theata_linear;
    j_linear_cv(i)=mean((h-y_cv).^2);
...
for i=1:5000
    ...
    sum_cost =train_quad'*(train_quad*theata_quad-y_train);
    theata_quad=theata_quad-alpha*(sum_cost/m);   
    h = train_quad*theata_quad;
    j_quad_train(i)=mean((h-y_train).^2);    
    
end


for i=1:5000  
    sum_cost =alpha*(cv_quad'*(cv_quad*theata_quad-y_cv));
    theata_quad=theata_quad-sum_cost/n;   
    h = cv_quad*theata_quad;
    j_quad_cv(i)=mean((h-y_cv).^2);
...
for i=1:5000
    ...
    sum_cost =train_cube'*(train_cube*theata_cube-y_train);
    theata_cube=theata_cube-alpha*(sum_cost/m);   
    h = train_cube*theata_cube;
    j_cube_train(i)=mean((h-y_train).^2); 
     
end

for i=1:5000  
    sum_cost =alpha*(cv_cube'*(cv_cube*theata_cube-y_cv));
    theata_cube=theata_cube-sum_cost/n;   
    h = cv_cube*theata_cube;
    j_cube_cv(i)=mean((h-y_cv).^2);
...
j_qad_cv=[];
 ...
 for i=1:5000
    ...
    sum_cost =train_qad'*(train_qad*theata_qad-y_train);
    theata_qad=theata_qad-alpha*(sum_cost/m);   
    h = train_qad*theata_qad;
    j_qad_train(i)=mean((h-y_train).^2); 
...
for i=1:5000  
    sum_cost =alpha*(cv_qad'*(cv_qad*theata_qad-y_cv));
    theata_qad=theata_qad-sum_cost/n;   
    h = cv_qad*theata_qad;
    j_qad_cv(i)=mean((h-y_cv).^2);
